...
VM_PROCESSOR_EFFECTIVE_SPEED = Gauge("vmware_vm_processor_effective_speed", "VM Qty of MHz this vm is using",
...
VM_MEMORY_BALLOONED = Gauge("vmware_vm_ballooned", "VM Size of Balloon in MB", registry=PROM_REGISTRY)
...
def populate_memory_data():
    ...
    VM_MEMORY_BALLOONED.set(GUEST_LIB.get_mem_ballooned_mb())
...
def populate_processor_data():
    ...
    VM_PROCESSOR_EFFECTIVE_SPEED.set(effective_mhz)
...
if __name__ == '__main__':
    ...
    start_http_server(ARGS.port, registry=PROM_REGISTRY)
    while True:
        try:
            populate_data()
